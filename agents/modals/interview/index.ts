...
import { BaseTools, FaqTools, LanguageTools, ServiceTools } from '../../tools'
...
const SYSTEM_PROMPT =
  ...
  'Dùng công cụ get_services để giới thiệu các dịch vụ khi khách hỏi, get_contact_info khi cần thông tin liên hệ, ' +
  'và get_faq khi khách hỏi các câu hỏi thường gặp.'
  ...
  constructor() {
    super({
      ...
      tools: BaseTools.compose(
        new ServiceTools({ only: [TOOL_NAME.getServices, TOOL_NAME.getContactInfo, TOOL_NAME.getBranches] }),
        new FaqTools(),
        new LanguageTools()
      ),
    })
